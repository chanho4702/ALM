...
- 남음: **이메일 알림**(SMTP 서버 구성 선행), 실시간 푸시(WebSocket/Web Push)
...
### 페이지 손질 이어서 (프론트 단독 가능)
- [ ] 보드 화면 — 카드/컬럼 밀도·헤더 손질 (지라 참고)
- [ ] 홈 화면 — 지라 "For you" 스타일 구성(최근 작업·내 이슈 위주)
- [ ] 리포트 화면 정리

### 백엔드 의존
- [ ] 이메일 알림 — SMTP(메일 서버) 구성 선행, §3의 잔여분
- [ ] 휴지통 자동 비우기 — 보존 기간 경과 프로젝트 영구 삭제 스케줄러
