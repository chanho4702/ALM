...
4. **설정 시스템 잔여** — 설계 확정(specs/2026-07-18-workflow-design.md v2): **공통(워크스페이스)+프로젝트 커스텀 2계층 상속 모델**, 카테고리 불변 전략. 구현 순서: 모델·resolveSettings → B(이슈 타입 탭) → A(워크플로). 워크로그(C)는 완료
