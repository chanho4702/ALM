...
- 테스트 **413 케이스 / 46 파일** — 스토어 단위 + REST 계약 + Testing Library 통합(App 전체 마운트)
...
해결(Resolution)은 완료했다(2026-08-29, 서버 V6 포함). 버전/릴리스도 완료했다(2026-08-30, 서버 V7 포함). 첨부파일도 완료했다(2026-08-30, 서버 V8 + MinIO). 설정 체계 재구성(2026-08-30): 설정 전용 사이드바 페이지, 전역 상태 카테고리·상태·이슈 타입 레지스트리(목업 전용 — 서버 저장은 백엔드 후속), 워크플로 캔버스 편집(dagre), 간트 SVAR 교체.
