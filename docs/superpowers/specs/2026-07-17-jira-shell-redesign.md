...
## v2 보완 (2026-07-17): 새 지라 내비게이션 — 전역 사이드바 상주

사용자 피드백: 실제 Jira Cloud(새 나비, 2024+)는 사이드바가 프로젝트 내부 전용이 아니라
**모든 화면에 상주하는 전역 사이드바**다 (`/jira/for-you` 홈 참고).

**새 지라 전역 사이드바 구조:**
- 상단: 홈(For you) · 최근 · 별표 …
- "프로젝트" 섹션: 프로젝트 목록, **현재 프로젝트는 펼쳐져 하위 항목(보드/백로그/…)이 중첩**
- 상단바는 검색 · 만들기 · 알림 · 프로필만

**v2 재설계:**
- `GlobalSideNav`(신규, 커스텀+토큰): 항상 표시. 홈 / 프로젝트 + "프로젝트" 섹션(목록,
  현재 프로젝트 확장: 대시보드/보드/백로그/이슈/설정)
- `HomePage`(`/home`, 앱 홈): 내 담당 이슈(전 프로젝트) + 최근 업데이트
- `ProjectLayout`: 자체 SideNav 제거 — 브레드크럼 + Outlet만 (사이드바는 전역이 담당)
- TopBar의 "프로젝트 ▾" 드롭다운 제거 (사이드바로 이관), 만들기/검색/사용자 유지
- 라우팅: `*` → `/home`
