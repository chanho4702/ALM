...
- **프로젝트 다중** — 생성/전환, 이슈 키 접두어(`ALM-1`, 삭제돼도 번호 재사용 안 함)
- **칸반 보드** — 할 일/진행 중/완료 3컬럼, @dnd-kit 드래그 이동 (활성 스프린트 이슈만 표시)
- **백로그/스프린트** — 생성·시작·완료(미완료 이슈 백로그 복귀), 인라인 이슈 생성, Dropdown으로 스프린트 이동/삭제
- **이슈 목록** — 테이블 + 필터(검색/상태/우선순위/담당자)
- **이슈 상세** — `?issue=ALM-1` URL 공유 모달: 제목 인라인 편집, 설명, 속성 패널(Select 4종), 코멘트/활동로그 Tabs
...
```
src/
├── app/                # 라우터(/projects/:projectId/board|backlog|issues), 전역 스타일
├── features/jira/
